...
const Login = () => {
  ...
  // Step 1: Submit credentials — first try without face
  const onSubmit = async (data) => {
    ...
    // If backend says face is required, move to step 2
    if (result.faceRequired) {
      setStep(2);
      return;
    }
  ...
  const handleFaceLogin = async () => {
    ...
    setFaceError(null);
    const result = await login(credentials, faceImage);
    ...
    if (result.faceVerificationFailed) {
      setFaceError(`Face mismatch detected (${Math.round((result.confidence || 0) * 100)}% confidence). Please try again.`);
      setFaceImage(null);
    }
  };
      ...
      <div className="flex-1 flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-20 xl:px-24 bg-white">
        <div className="mx-auto w-full max-w-sm lg:max-w-md">
          ...
          <div className="text-center">
            ...
            {/* Step indicator for face verification */}
            {step === 2 && (
              <div className="flex items-center justify-center gap-3 mb-4">
                ...
                <div className="flex items-center gap-2 px-3 py-1.5 rounded-full text-sm font-medium bg-indigo-100 text-indigo-700">
                  <span className="w-5 h-5 bg-indigo-600 text-white rounded-full text-xs flex items-center justify-center">2</span>
                  Face ID
                </div>
              </div>
            )}
            ...
            <p className="mt-2 text-sm text-gray-600">
              {step === 1 ? (
                ...
                "Your account requires face verification to log in"
              )}
            </p>
          </div>
            ...
            <div className="mt-8 space-y-6">
              ...
              <div className="bg-gradient-to-r from-indigo-50 to-purple-50 border border-indigo-200 rounded-xl p-4">
                <div className="flex gap-3">
                  ...
                  <div>
                    <h4 className="font-semibold text-indigo-800 text-sm">Identity Verification</h4>
                    <p className="text-indigo-700 text-xs mt-1">
                      Your credentials were verified. Now confirm your identity by looking at the camera.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <button
                type="button"
                onClick={() => { setStep(1); setFaceImage(null); setFaceError(null); }}
                className="w-full py-2.5 px-4 text-sm font-medium text-gray-600 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors"
              >
                ← Back to credentials
              </button>
            </div>
